...
import _modules.listObject_hyp_2 as listObject
...
def yieldObject(rowData: DataFrame, years: list[int, int], numDropped: int) -> listObject.entry:
    ...
    headers = ['Operating revenue (Turnover)\nth USD ', 'Total assets\nth USD ']
    if (res:= refData(rowData[3:], years, headers)):
        revenue = res['Operating revenue (Turnover)\nth USD ']
        ...
        itm = listObject.entry(ID=id, name=name, NAICE=naice, totAssets=totAssets, revenue=revenue)
        return itm
    else:
        numDropped += 1
        return numDropped
...
def getRevBySector(sectorData, years):
    revenueBySector = defaultdict(lambda: {'laggedAssets': np.empty(years[0]-years[1]), 'opRev': np.empty(years[0]-years[1])})
    for i in list(sectorData.keys()):
        for entry in sectorData[i]:
            for yearIdx in range(len(entry.laggedAssets)):
                if not np.isnan(entry.laggedAssets[yearIdx]) and not np.isnan(entry.opRev[yearIdx]):
                    revenueBySector[i]['laggedAssets'][yearIdx] += float(entry.laggedAssets[yearIdx])
                    revenueBySector[i]['opRev'][yearIdx] += float(entry.opRev[yearIdx])
    revenueBySector = dict(revenueBySector)
    revenueScaledbySector = defaultdict(lambda: defaultdict(list))
    for i in list(revenueBySector.keys()):
        revenueScaledbySector[i] = revenueBySector[i]['opRev']/revenueBySector[i]['laggedAssets']
    return dict(revenueScaledbySector)
...
if __name__ == '__main__':
    ...
    revenueBySectorScaled = getRevBySector(sectorData, years)
    for i in list(revenueBySectorScaled.keys()):
        revenueBySectorScaled[i] = list(revenueBySectorScaled[i])
    
    # save to file
    if saveAsJSON(revenueBySectorScaled, oFilePath):
        print("__DONE__")
